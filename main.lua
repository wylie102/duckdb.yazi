...
local function generate_sql(job, mode)
	if mode == "standard" then
		-- In standard mode select all rows from the file with a dynamic LIMIT and OFFSET.
		...
		-- Summarize mode: use detailed summarization query.
		return [[
        SELECT
        ...
        FROM (summarize FROM ']] .. tostring(job.file.url) .. [[');
        ]]
	end
end
...
function M:seek(job)
	if (os.getenv("DUCKDB_PREVIEW_MODE") or "summarize") == "standard" then
